...
import { DENSE_ENTITY_ID, denseDocuments, denseEntity } from './prototype-dense';

export const allEntities: readonly Entity[] = [...corpus.entities, denseEntity];

export function findEntity(id: string): Entity | undefined {
  if (id === DENSE_ENTITY_ID) return denseEntity;
  return corpus.entities.find((entity) => entity.id === id);
}

export function findRelation(id: string): Relation | undefined {
  return corpus.relations.find((relation) => relation.id === id);
}
...
export function findDocument(id: DocId): DocumentRow | undefined {
  return (
    corpus.documents.find((document) => document.id === id) ??
    denseDocuments.find((document) => document.id === id)
  );
}
...
export function directRelations(entityId: string): Relation[] {
  return corpus.relations.filter(
    (relation) =>
...
export function pendingProposals(entityId: string): Proposal[] {
  return corpus.proposals.filter(
...
export function promotingProposal(promotedFrom: string): Proposal | undefined {
  return corpus.proposals.find((proposal) => proposal.id === promotedFrom);
}
