...
export function ClaimRow({
  ...
  return (
    <div className="grid grid-cols-[10rem_minmax(0,1fr)_auto] items-center gap-2 border-b border-border/60 py-0.5 hover:bg-muted/40">
  ...
  return (
    <div className="space-y-1 text-xs">
      ...
      <dl className="grid grid-cols-[5.5rem_minmax(0,1fr)] gap-x-2">
        ...
        <dd>{document.retrievedAt ?? <span className="text-muted-foreground">not recorded</span>}</dd>
      </dl>
    </div>
  );
}
...
export function LabellingBanner() {
  return (
    <p className="border-y border-border bg-muted/50 px-2 py-1 text-[0.7rem] text-muted-foreground">
      Candidate claims. A score belongs to the source document, never to the claim (S1). The
      scoring is automated and unmeasured (S3). Placeholder words — #12 writes the real ones.
    </p>
  );
}
...
export function PendingProposalRow({ proposal, badges }: { proposal: Proposal; badges: ReactNode }) {
  return (
...
export function ProvenanceFooter({ entity }: { entity: Entity }) {
  ...
  return (
    <p className="text-[0.7rem] text-muted-foreground">
      ...
      . The stored rendered prompt of that call is not shown: it holds retrieved source text and
      #45 is open.
    </p>
  );
}
