...
/**
 * The same value with no box at all. **The shape of the text is the type**: a number is
 * monospace and aligned right, a date is monospace, a boolean is a mark and not a word, a list
 * carries its count. Nothing writes the name of a type.
 */
export function ValueText({ attribute }: { attribute: Attribute }) {
  const shape = shapeOf(attribute.v);

  if (shape.kind === 'boolean') {
    return (
      <span className="font-mono" title={shape.value ? 'true' : 'false'}>
        {shape.value ? '✓' : '—'}
      </span>
    );
  }
  ...
  if (shape.kind === 'number') {
    return <span className="block truncate text-right font-mono tabular-nums">{shape.value}</span>;
  }

  if (shape.kind === 'date') {
    return <span className="block truncate font-mono">{shape.value}</span>;
  }

  if (shape.kind === 'list') {
    // No count. It was drawn beside the last item and read as part of the value.
    return (
      <span className="block truncate" title={shape.value.join(', ')}>
        {shape.value.join(', ')}
      </span>
    );
  }
  ...
  return (
    <span className="block truncate" title={shape.value}>
      {shape.value}
    </span>
  );
}
...
/** One claim, one line: the name, the field, the badges. */
export function ClaimRow({
  ...
  return (
    <div
      className={`grid items-center gap-2 py-0.5 hover:bg-muted/40 ${
        compact ? 'grid-cols-[6.5rem_minmax(0,1fr)_auto]' : 'grid-cols-[11rem_18rem_auto]'
      }`}
    >
