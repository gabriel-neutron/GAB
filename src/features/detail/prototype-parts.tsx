...
const FIELD =
  ...
  // A number field reserves room for a spinner it will never show, and that room was eating the
  // last two digits of a tonnage.
...
function cellWidth(attribute: Attribute): string {
  const shape = shapeOf(attribute.v);
  if (shape.kind === 'boolean') return 'basis-[17rem]';

  // A date is ten characters and a browser draws it as `dd/mm/yyyy` with its own padding, so it
  // never fits the narrow cell that its length suggests.
  if (shape.kind === 'date') return 'basis-[20rem]';

  const length = shape.kind === 'list' ? shape.value.join(', ').length : String(shape.value).length;
  if (length <= 12) return 'basis-[17rem]';
  if (length <= 34) return 'basis-[26rem]';
  return 'basis-full';
}
...
export function ClaimCell({
  ...
  return (
    ...
    >
      <span
        className={`shrink-0 truncate text-xs text-muted-foreground ${compact ? 'w-24' : 'w-32'}`}
