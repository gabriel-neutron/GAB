...
import { boundsOfAll, byFid, entityTypes, geoEntities, type GeoEntity } from './prototype-corpus';
...
// ------------------------------------------------------------------------------------------
// The workspace. ADR 0004 §7: the camera and the layer visibility are workspace, not identity,
// so they live in `localStorage` and never in the URL. The key names the prototype, so the real
// feature starts from an empty one. This is use case 5, and it reports to #33.
// ------------------------------------------------------------------------------------------
...
const WORKSPACE_FALLBACK: Workspace = { lon: 30, lat: 58, zoom: 3, hidden: [] };
...
export function mountMap(host: HTMLElement, colourOf: (type: string) => string): MapHandle {
  const saved = readWorkspace('map-prototype', isWorkspace, WORKSPACE_FALLBACK);
  ...
  const applySelection = (entity: GeoEntity | null): void => {
    selection = entity;
    setSourceData('selected', entity === null ? [] : [entity]);
    for (const listener of selectListeners) listener(entity);

    // Use case 4: the map's only output. The detail sidebar is built in another session and in
    // another feature, and a feature never imports another feature (ADR 0004 §5). An event on
    // `window` is the cheapest seam a prototype can offer, and it is **not** a proposal for the
    // real one — the seam of ADR 0004 §5 carries the selection, and it is not written yet.
