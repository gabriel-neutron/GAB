...
import { ChangeMark, KIND_ACCENT } from './review-parts';
    ...
    <div className="max-w-3xl">
      ...
      <ul className="mt-6">
        {rows.map((row) => (
          <li
            key={row.proposal.id}
            className={`flex items-baseline gap-3 border-l-2 py-2 pl-3 ${KIND_ACCENT[row.kind]}`}
          >
            ...
            <span className="flex-1">
              ...
              <span className="text-xs text-muted-foreground">
                ...
                {row.proposal.decidedAt?.slice(0, 10) ?? 'no date'} ·{' '}
                {/* An operator edit still carries an `agentCall`, and that call names an agent
                    that never ran. The writing role is the only honest reading here. */}
                {row.proposal.authorRole === 'gabriel_app'
                  ? 'by hand'
                  : (row.agentLabel ?? 'no agent named')}
              </span>
            </span>
