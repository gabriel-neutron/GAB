...
/**
 * **One line, and one link per document.**
 *
 * The operator refused two links on 11 August 2026. The title is the link, and it points at the
 * **archive copy** when one exists, because that is the copy taken at ingest and it cannot drift
 * or disappear; the original address is the fallback. A document with neither — a scan in the
 * bucket — is not a link at all, because #31 keeps the bucket private and there is nothing a
 * reader may be given.
 *
 * **Which of the two a reader gets is #31's to settle, not this screen's.** The choice above is
 * the prototype's, and it is reported.
 */
export function SourceLink({ source }: { source: ResolvedSource }) {
  ...
  return (
    <span className="inline-flex items-baseline gap-1.5">
      {href === null ? (
        <span title="No address. The bucket is private — #31.">{doc.title}</span>
      ) : (
        <a
