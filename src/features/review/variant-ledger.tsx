...
import { ageLabel, formatValue, short, type Change, type Subject } from './review-model';
      ...
      {current === null ? null : (
        <div className="flex min-h-0 flex-col">
          ...
          <div className="mt-4 min-h-0 flex-1 overflow-y-auto pr-2">
            {/* A measure, not the width of the window: at 1600px the columns drift so far apart
                that the alignment the table bought is lost again. */}
...
function Head({ subject }: { subject: Subject }) {
  ...
  return (
    <header className="flex max-w-5xl items-baseline justify-between gap-4 border-b border-border pb-2">
      <div>
      ...
      <tr className={`border-b border-border ${tint} ${settled}`}>
        ...
        <td colSpan={6} className="pb-2 pl-0">
          <div className="space-y-1">
            ...
            <GapLine>
              {change.authorLabel} · {ageLabel(change.ageDays)}
            </GapLine>
