  ...
  const render = (): void => {
    ...
    for (const type of model.entityTypes) {
      ...
      label.append(
        ...
        el('span', 'w-6 shrink-0 text-right font-mono text-[11px] text-muted-foreground', on ? 'on' : 'off'),
      );
      ...
      if (matches.length === 0) {
        list.append(el('p', 'py-1 pl-6 pr-2 text-[11px] text-muted-foreground', 'Nothing matches.'));
      }
    }
    host.append(list);

    // ---- the controls the map has no counterpart for -----------------------------------------
    const extras = el('div', 'flex flex-wrap items-center gap-1.5 border-t border-border px-2 py-2');
    extras.append(
    ...
    const degreeWrap = el('label', 'flex w-full items-center gap-2 text-[11px] text-muted-foreground');
...
/** What the paint means. One encoding per macro read the analyst asked for. */
