...
export interface GraphPageProps {
  /** PROTOTYPE — the direction vocabulary the address asked for. `shared/direction.prototype.ts`. */
...
export function GraphPage({ variant }: GraphPageProps) {
  ...
  useEffect(() => {
    ...
    // PROTOTYPE: the variant is in the list, so a change of it mounts the graph again. The edge
    // program is a setting of the instance, so it is read one time, at the mount. The list is
    // empty again when `shared/direction.prototype.ts` goes.
  }, [variant]);
