 ...
 *
 * **The operator asked for one visual vocabulary across the graph and the map.** They liked the
 * hover name of the graph and asked for it on the map, so that the two surfaces read as one
 * product.
 *
 * **The two canvases are drawn by different libraries**, Sigma and MapLibre, so no draw call is
 * shared and no React component can own this element: the DOM over a live canvas belongs to the
 * adapter of that canvas. What is shared is this file — the words of the class list and the
 ...
 * carried a direction all along and neither canvas drew it. **The direction on the line itself is
 * a different job**: this names a relation, and the line draws one.
 *
 * **A third caller reads it, and it is not a canvas.** The detail view of a relation states
 ...
 *
 * **One rule, for the two canvases and for the detail surface**, and the operator ruled it.
 ...
 *
 * **The count is the words that the size owes a reader.** The graph sizes a node by its degree,
 * and a size alone is unreadable to anybody who cannot compare two discs: words belong beside the
 * hue, and the same argument reaches the radius. The map draws every
