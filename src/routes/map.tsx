...
import { BAR_NAMES, MapPage, nextBar, parseBar, type BarKey } from '@/features/map/map-page';
 ...
 *
 * `?variant=` is prototype scaffolding: it chooses the left bar, and it dies with the prototype.
...
export const Route = createFileRoute('/map')({
  ...
  validateSearch: (search: Record<string, unknown>): MapSearch => ({
    variant: parseBar(search['variant']),
...
/**
 * **PROTOTYPE — throwaway.** The floating bar that flips between the three left bars. It is
...
function PrototypeSwitcher({
  ...
  readonly bar: BarKey;
  readonly onSelect: (bar: BarKey) => void;
}) {
  useEffect(() => {
    const onKey = (event: KeyboardEvent): void => {
      if (event.key !== 'ArrowLeft' && event.key !== 'ArrowRight') return;

      // An arrow key inside a field belongs to the field. Every left bar carries one.
      ...
      onSelect(nextBar(bar, event.key === 'ArrowLeft' ? -1 : 1));
    };
  ...
  return (
    <div className="fixed right-3 bottom-3 z-50 flex items-center gap-1 border border-neutral-600 bg-neutral-950 p-1 font-mono text-xs text-neutral-100 shadow-lg">
      <button
        type="button"
        className="px-2"
        aria-label="Previous left bar"
        onClick={() => {
          onSelect(nextBar(bar, -1));
        }}
      >
        {'<'}
      </button>
      <span className="min-w-[16rem] text-center">{`${bar} — ${BAR_NAMES[bar]}`}</span>
      <button
        type="button"
        className="px-2"
        aria-label="Next left bar"
        onClick={() => {
          onSelect(nextBar(bar, 1));
        }}
      >
