...
function MapRoute() {
  ...
  // A stale identifier is as good as none: the sidebar stays away rather than reporting a fault
  // the operator cannot act on. The read layer replaces this line when #26 lands.
  const resolved = entity !== '' && corpus.entities.some((row) => row.id === entity);
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 flex overflow-hidden">
        <MapPage row={variant} />
        {/* No selection, no sidebar. `DetailSidebar` draws its own index when it is handed an
            identifier it cannot resolve, and that page belongs to `/entity/:id`, where a bad
            address is the fault. Here an empty address is the normal state of a map, so the
            route answers by composing nothing. The map keeps the width. */}
        {resolved && <DetailSidebar entityId={entity} />}
      </div>
      <PrototypeSwitcher
        row={variant}
        onSelect={(next) => {
