...
role with no privilege on `public.entities` wrote a row through an ordinary `api` view. The perimeter is a blanket
...
every apply, so a convenience grant is erased rather than inherited. Each view also carries
