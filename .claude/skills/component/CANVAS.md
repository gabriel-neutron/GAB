...
everything below it, and runs its own loop. The map and the graph are the two that exist today. A
third such library is a question for the operator before it is installed, and these rules hold for
it from that day.
...
- **One `ref`, one imperative adapter, and every other value outside React.** No React state and no
  React render inside the tree that wraps the live element. ADR 0004.
...
- **The adapter publishes a handle, and every other component drives the library through it.** The
  adapter is the one file that touches the library.
...
- **No story mounts a live canvas.** One story is one live WebGL context, and a browser drops the
  oldest after about sixteen. The Storybook configuration records it. Story the panels beside the
  canvas, and check the canvas itself in the running application.
...
- **A source lookup returns nothing before the style loads, and it returns it in silence.** A
  selection restored from the address is applied again once the style is loaded.
- **A component that subscribes after the canvas is built has already missed the restore.** Seed
...
- **A CSS custom property never reaches a map or a graph style parser.** The hues are copied as hex,
  the copy lives in the derivation of that surface with the reason beside it, and the copy is a
  recorded cost.
- **The dark set is used**, because a point sits on imagery and the light set cannot be read on it.
- The theme is read from the class on the document element, with an observer, and never from React.

## Data and layers

- **One data source, and one layer per type.** A panel of thirty types must not be thirty queries. A
  type is hidden with a visibility property, which needs no new data, and a hidden layer returns
  nothing from a hit test.
...
- **A point wins over a line it crosses**, and a line needs a hit box of about 5px on each side.
  Measure the hit box in the browser: a library states the thickness of a pick band in its own
  units, and the number it takes is not always the half-width.
