...
Run the project check command. No document names one today, and the stack is open: if you find no check command, do not invent one — report `CHECK: none`. Do not commit; the operator reads the diff. Report in 15 lines or fewer: RESULT (DONE, BLOCKED or PARTIAL); FILES (each path, new or changed, one reason for each); CHECK (the command and its status, or none); RULES (each failed test, or none); ASSUMED (each assumption, or none); ASK (each question, or none).
